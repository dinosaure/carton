...
module Make
    ...
    (Uid : Carton.UID) : sig
  val make : Store.t -> idx:(Store.uid -> Store.uid) -> (Store.uid, Store.fd, Uid.t) t IO.t
  ...
  val get
    :  Store.t
    -> resources:((Store.fd * int64) -> ((Store.fd * int64) buffers -> Carton.Dec.v IO.t) -> Carton.Dec.v IO.t)
    -> (Store.uid, Store.fd, Uid.t) t
    -> Uid.t
