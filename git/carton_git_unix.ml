...
module Store = struct
  ...
  type uid = Fpath.t and fd = Lwt_unix.file_descr
  ...
  let create root path =
    ...
    let process () =
      Lwt_unix.openfile (Fpath.to_string path) Unix.[ O_RDONLY ] 0o644 >>= fun fd ->
      Lwt.return_ok fd in
    let error = function
      | Unix.Unix_error (Unix.EACCES, _, _) -> Lwt.return_error (`Not_found path)
      | exn -> Lwt.fail exn in
    Lwt.catch process error

  let map : t -> fd -> pos:int64 -> int -> Bigstringaf.t fiber = fun _ fd ~pos len ->
    let fd = Lwt_unix.unix_file_descr fd in
    let payload = Mmap.V1.map_file fd ~pos Bigarray.char Bigarray.c_layout false [| len |] in
  ...
  let list root =
    ...
    let rec go acc =
      Lwt.catch
        (fun () -> Lwt_unix.readdir dh >>= function
           | "." | ".." -> go acc
           | entry -> match Fpath.of_string entry with
             | Ok x ->
               if Fpath.has_ext "pack" x then go (x :: acc)
               else go acc
