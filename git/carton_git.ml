...
type ('path, 'fd, 'uid) t =
  { tbl : ('path, ('fd, 'uid) pack) Hashtbl.t } [@@unbox]
...
module Make
  ...
  let idx (root : Store.t) acc path =
    ...
    let idx = Carton.Dec.Idx.make payload
        ~uid_ln:Uid.length
        ~uid_rw:Uid.to_raw_string
  ...
  let pack (root : Store.t) acc (index, pack) =
    ...
    let pack = Carton.Dec.make (fd, length) ~z ~allocate:(fun _ -> w)
      ~uid_ln:Uid.length
      ~uid_rw:Uid.of_raw_string
      (fun uid -> match Carton.Dec.Idx.find index uid with
  ...
  let make : Store.t -> idx:(Store.uid -> Store.uid) -> (Store.uid, Store.fd, Uid.t) t IO.t
    = fun root ~idx:map ->
      Store.list root >>= fun pcks ->
      let idxs = List.map map pcks in
      fold_left_r (idx root) [] idxs >>| List.rev >>= fun idxs ->
      fold_left_r (pack root) [] (List.combine idxs pcks) >>| List.rev >>= fun vs ->
  ...
  let add
    : Store.t -> (Store.uid, Store.fd, Uid.t) t -> idx:(Store.uid -> Store.uid) -> Store.uid -> (unit, Store.error) result IO.t
  ...
  let with_resources root pack uid buffers =
    ...
    Carton.Dec.weight_of_uid io ~map pack ~weight:Carton.Dec.null uid |> Scheduler.prj >>= fun weight ->
    let raw = Carton.Dec.make_raw ~weight in
    Carton.Dec.of_uid io ~map pack raw uid |> Scheduler.prj >>= fun v ->
    return v

  let get
    : Store.t -> resources:('fd -> ('fd buffers -> 'a IO.t) -> 'a IO.t) -> (Store.uid, Store.fd, Uid.t) t -> Uid.t -> (Carton.Dec.v, [> `Msg of string ]) result IO.t
    = fun root ~resources p uid ->
      let res = ref None in
      Hashtbl.iter (fun k { index; _ } -> if Carton.Dec.Idx.exists index uid then res := Some k) p.tbl ;
      match !res with
      | Some k ->
        let { pack; _ } = Hashtbl.find p.tbl k in
        resources (Carton.Dec.fd pack) (with_resources root pack uid) >>= fun v ->
        return (Ok v)
      | None -> return (Error (`Not_found uid))

  let list
    : Store.t -> (Store.uid, Store.fd, Uid.t) t -> Uid.t list
    = fun _ p ->
      let fold _ { index; _ } a =
        let res = ref [] in
        Carton.Dec.Idx.iter ~f:(fun ~uid ~offset:_ ~crc:_ -> res := uid :: !res) index ;
