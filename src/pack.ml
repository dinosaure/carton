...
sig
  ...
  val deltas:
    ...
    -> int
    -> int
    -> ((Entry.t * t) list, error) result Monad.t
end
...
struct
  ...
  let rec _pp_delta ppf = function
    ...
    | S { length; depth; hunks; src; src_length; _ } ->
      Fmt.pf ppf "(Δ { @[<hov>length = %d;@ \
                              ...
                              src_length = %Ld;@] }"
        length depth
        (Fmt.hvbox (Fmt.Dump.list Rabin.pp)) hunks
        (Fmt.hvbox _pp) src
        src_length
  and _pp ppf { delta; } =
    Fmt.pf ppf "{ @[<hov>delta = %a;@] }"
  ...
  let deltas ?(memory = false) entries get tag window max =
    let to_delta e = match Entry.delta e, Entry.preferred e with
      ...
      | (Entry.From _ | Entry.None), _  -> false in

    let tries =
      List.filter to_delta entries
      |> List.stable_sort Entry.compare in

    let untries =
      List.filter (fun x -> not (to_delta x)) entries
      |> Entry.topological_sort in

    let window_pack =
      if memory
      then (module MemoryCache  : WINDOW)
      else (module ElementCache : WINDOW) in
    ...
    let normalize lst =
      Mr.map (fun trg_entry -> match Entry.delta trg_entry with
          | Entry.None ->
            Monad.return (trg_entry, { delta = Z })
          | Entry.From hash ->
         ...
         Mr.fold
           (fun (window, acc) entry ->
              ...
              | Some raw ->
                ...
                | None -> Monad.return (window, (entry, base) :: acc)
                | Some (src_entry, hunks, length) ->
                  ...
                  | Some ((src, _, _), window) ->
                    ...
                    Monad.return (window, (Entry.with_delta entry Entry.(From (id src_entry)), base) :: acc)
                  | None -> Monad.return (window, (entry, base) :: acc))
           (window, []) tries)
      (fun (_, tries) ->
         Monad.try_bind
           (fun () -> normalize untries)
           (fun untries -> List.append tries untries |> sort tag |> ok |> Monad.return)
           (function Uncaught_hash hash -> Error (Invalid_hash hash) |> Monad.return
                   | exn -> Monad.fail exn)) (* XXX(dinosaure): could we have a better choice? *)
      (function Uncaught_hash hash -> Error (Invalid_hash hash) |> Monad.return
              | exn -> Monad.fail exn) (* XXX(dinosaure): same as below. *)
end
...
struct
  ...
  type t =
    ...
    ; i_len   : int
    ; write   : int64
    ; radix   : (Checksum.t * int64) Radix.t
  ...
  struct
    ...
    let header kind len checksum k dst t =
      ...
      let rec loop idx checksum dst t =
        if idx < !pos
        then
          let byte = Char.code (Bytes.get tmp_header idx) in
          let checksum = Checksum.digestc checksum byte in

          put_byte byte (loop (idx + 1) checksum) dst t
        else k checksum dst t
      in

      loop 0 checksum dst t

    let tmp_offset = Bytes.create 10

    let offset n checksum k dst t =
      let pos = ref 9 in
      let off = ref n in
      ...
      loop !pos checksum dst t

    let hash hash checksum k dst t =
      if t.o_len - t.o_pos >= Hash.Digest.length
      then begin
        let checksum = Checksum.digests checksum hash in

        Cstruct.blit_from_bytes hash 0 dst (t.o_off + t.o_pos) Hash.Digest.length;

        k checksum dst { t with o_pos = t.o_pos + Hash.Digest.length
                         ; write = Int64.add t.write (Int64.of_int Hash.Digest.length) }
      end else
        let rec loop rest checksum dst t =
          if rest = 0
          then k checksum dst t
          else
            ...
            if n = 0
            then flush dst { t with state = Hash (loop rest checksum) }
            else begin
              let checksum = Checksum.digests checksum ~off:(Hash.Digest.length - rest) ~len:n hash in

              Cstruct.blit_from_bytes hash (Hash.Digest.length - rest) dst (t.o_off + t.o_pos) n;

              loop (rest - n) checksum dst { t with o_pos = t.o_pos + n
                                             ; write = Int64.add t.write (Int64.of_int n) }
            end
        in

        loop Hash.Digest.length checksum dst t
  end
  ...
  let writek kind entry entry_delta rest dst t =
    ...
    | KindRaw, { Delta.delta = Delta.Z } ->
      ...
      (KWriteK.header (Kind.to_bin (Entry.kind entry)) (Entry.length entry) Checksum.default
       @@ fun checksum _ t ->
    ...
    | KindHash, { Delta.delta = Delta.S { length; hunks; src_length; src_hash; _ } } ->
      ...
      (KWriteK.header 0b111 (Int64.of_int length) Checksum.default
       @@ fun checksum -> KWriteK.hash (Hash.to_string src_hash |> Bytes.unsafe_of_string) checksum
       @@ fun checksum _ t ->
       let z = Deflate.default 4 in
       let z = Deflate.flush (t.o_off + t.o_pos) (t.o_len - t.o_pos) z in
    ...
    | KindOffset, { Delta.delta = Delta.S { length; hunks; src_length; src_hash; _ } } ->
      (match Radix.lookup t.radix src_hash with
       | Some (_, src_off) ->
         ...
         (KWriteK.header 0b110 (Int64.of_int length) Checksum.default
          @@ fun checksum -> KWriteK.offset rel_off checksum
          @@ fun checksum _ t ->
          let z = Deflate.default 4 in
          let z = Deflate.flush (t.o_off + t.o_pos) (t.o_len - t.o_pos) z in
  ...
  let writez src dst t x r checksum off used_in z =
    match Deflate.eval ~src ~dst z with
      | `Await z ->
        await { t with state = WriteZ { x; r; checksum; off; ui = used_in; z; }
                     ; i_pos = Deflate.used_in z }
      | `Flush z ->
        let checksum = Checksum.digest ~off:(t.o_off + t.o_pos) ~len:(Deflate.used_out z) checksum dst in

        flush dst { t with state = WriteZ { x; r; checksum; off; ui = (Deflate.used_in z); z; }
      ...
      | `End z ->
        ...
        Cont { t with state = Save { x; r; checksum; off; }
                    ; o_pos = t.o_pos + (Deflate.used_out z)
                    ; i_pos = Deflate.used_in z
                    ; write = Int64.add t.write (Int64.of_int (Deflate.used_out z)) }
      | `Error (_, exn) -> error t (Deflate_error exn)

  let writeh src dst t ((entry, _) as x) r checksum off used_in h z =
    match Deflate.eval ~src:t.h_tmp ~dst z with
    | `Await z ->
      (match Hunk.eval src t.h_tmp h with
       | `Await h ->
         await { t with state = WriteH { x; r; checksum; off; ui = 0; z; h; }
       ...
       | `Flush h ->
         ...
         Cont { t with state = WriteH { x; r; checksum; off; ui; h; z; }
                     ; i_pos = Hunk.used_in h }
       | `End h ->
         let used_in' = used_in + Deflate.used_in z in
    ...
    | `Flush z ->
      ...
      flush dst { t with state = WriteH { x; r; checksum; off; ui = used_in'; h; z; }
                       ; i_pos = Hunk.used_in h
                       ; o_pos = t.o_pos + (Deflate.used_out z)
                       ; write = Int64.add t.write (Int64.of_int (Deflate.used_out z)) }
    | `End z ->
      ...
      Cont { t with state = Save { x = entry; r; checksum; off; }
                  ; o_pos = t.o_pos + (Deflate.used_out z)
                  ; i_pos = 0
                  ; i_len = 0
                  ; i_off = 0
  ...
  let save _ t x r checksum off =
    Cont { t with state = Object (iter r)
                ; radix = Radix.bind t.radix (Entry.id x) (checksum, off) }
  ...
  let eval src dst t =
    let eval0 t = match t.state with
      ...
      | Object k -> k dst t
      | WriteK k -> k dst t
      | WriteZ { x; r; checksum; off; ui; z; } -> writez src dst t x r checksum off ui z
      | WriteH { x; r; checksum; off; ui; h; z; } -> writeh src dst t x r checksum off ui h z
      | Save { x; r; checksum; off; } -> save dst t x r checksum off
      | Exception exn -> error t exn
      | Hash k -> k dst t
      | End hash -> Ok (t, hash)
    in
  ...
  let flush offset len t =
    ...
    then
      match t.state with
      | WriteZ { x; r; checksum; off; ui; z; } ->
        { t with o_off = offset
               ; o_len = len
               ; o_pos = 0
               ; state = WriteZ { x; r; checksum; off; ui; z = Deflate.flush offset len z } }
      | WriteH { x; r; checksum; off; ui; h; z; } ->
        { t with o_off = offset
               ; o_len = len
               ; o_pos = 0
               ; state = WriteH { x; r; checksum; off; ui; h; z = Deflate.flush offset len z } }
    ...
    then
      ...
      | WriteZ { x; r; checksum; off; ui; z; } ->
        { t with i_off = offset
               ; i_len = len
               ; i_pos = 0
               ; state = WriteZ { x; r; checksum; off; ui; z = Deflate.no_flush offset len z } }
      | WriteH { x; r; checksum; off; ui; z; h; } ->
        { t with i_off = offset
               ; i_len = len
               ; i_pos = 0
               ; state = WriteH { x; r; checksum; off; ui; z; h = Hunk.refill offset len h } }
      | (Header _ | Object _ | WriteK _ | Save _ | Hash _ | End _ | Exception _) ->
        { t with i_off = offset
               ; i_len = len
               ; i_pos = 0 }
  ...
  let finish t =
    ...
    then match t.state with
      | WriteZ { x; r; checksum; off; ui; z; } ->
        { t with state = WriteZ { x; r; checksum; off; ui; z = Deflate.finish z } }
      | WriteH { x; r; checksum; off; ui; z; h; } ->
        { t with state = WriteH { x; r; checksum; off; ui; z; h = Hunk.finish h } }
...
module Stream
    ...
    (Mr: S.MR with type +'a m = 'a Monad.t)
= struct
  module Kind = Kind(A)(B)(C)(D)
  module Entry = Entry(A)(B)(C)(D)(Hash)(Kind)
  module Delta = Delta(A)(B)(C)(D)(Hash)(Kind)(Entry)(Monad)(Mr)
  module Hunk = Hunk(Hash)
  module Pack = Pack(A)(B)(C)(D)(Hash)(Checksum)(Deflate)(Kind)(Entry)(Delta)(Hunk)
