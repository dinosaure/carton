module Dec : sig
  ...
  val path_of_offset : map:'fd W.map -> ('fd, 'uid) t -> cursor:int64 -> path Lwt.t
  val path_of_uid : map:'fd W.map -> ('fd, 'uid) t -> 'uid -> path Lwt.t
  val of_offset_with_path : map:'fd W.map -> ('fd, 'uid) t -> path:path -> raw -> cursor:int64 -> v Lwt.t

  type 'uid digest = 'uid Carton.Dec.digest

  val uid_of_offset : map:'fd W.map -> digest:'uid digest -> ('fd, 'uid) t -> raw -> cursor:int64 -> (Carton.kind * 'uid) Lwt.t
  val uid_of_offset_with_source : map:'fd W.map -> digest:'uid digest -> ('fd, 'uid) t -> kind:Carton.kind -> raw -> depth:int -> cursor:int64 -> 'uid Lwt.t
  ...
  module Verify (Uid : Carton.UID) : sig
    ...
    val verify : threads:int -> map:'fd W.map -> oracle:Uid.t oracle -> ('fd, Uid.t) t -> matrix:status array -> unit Lwt.t
  end

  module Ip (Uid : Carton.UID) : sig
    val iter : threads:'a list -> f:('a -> uid:Uid.t -> offset:int64 -> crc:Idx.optint -> unit Lwt.t) -> Uid.t Idx.idx -> unit Lwt.t
  end
end
...
module Enc : sig
  ...
  module Delta (Uid : UID) (Verbose : VERBOSE) : sig
    val delta :
      ...
      weight:int ->
      uid_ln:int ->
      Uid.t entry array -> Uid.t q array Lwt.t
  end
