...
let pp_path ppf path =
  let[@warning "-8"] x :: r = Cd.path_to_list path in
...
let idx_of_pack fpath =
  ...
  | true ->
    ...
    (fun () -> Unix.close fd),
    (fun uid -> match Cd.Idx.find idx uid with Some (_, off) -> off | None -> raise (Not_found uid))
...
let of_off ~(digest:Uid.t Cd.digest) off fpath =
  let open Rresult.R in
  let fd = Unix.openfile (Fpath.to_string fpath) Unix.[ O_RDONLY ] 0o644 in
  let mx = let ic = Unix.in_channel_of_descr fd in in_channel_length ic in
  idx_of_pack fpath >>= fun (close_idx, idx) ->
  let pack = Cd.make { fd; mx; } ~z ~allocate ~uid_ln:Uid.length ~uid_rw:Uid.of_raw_string idx in

  let fiber () =
    let ( >>= ) = unix.Cd.bind in
    Cd.weight_of_offset unix ~map:unix_map pack ~weight:Cd.null ~cursor:off >>= fun weight ->
    Cd.path_of_offset unix ~map:unix_map pack ~cursor:off >>= fun path ->
    let raw = Cd.make_raw ~weight in
    Cd.of_offset_with_path unix ~map:unix_map pack ~path raw ~cursor:off >>= fun v ->
    let uid = digest ~kind:(Cd.kind v) ~off:0 ~len:(Cd.len v) (Cd.raw v) in
    pp_stat Fmt.stdout
      Uid.pp uid off (weight :> int) (Cd.len v) pp_kind (Cd.kind v) pp_path path ;
    Unix.close fd ; close_idx () ; unix.Cd.return () in
...
let of_uid uid fpath =
  ...
  let pck = Cd.make { fd= fd1; mx= mx1; } ~z ~allocate ~uid_ln:Uid.length ~uid_rw:Uid.of_raw_string
  ...
  let fiber () =
    let ( >>= ) = unix.Cd.bind in
    Cd.weight_of_uid unix ~map:unix_map pck ~weight:Cd.null uid >>= fun weight ->
    Cd.path_of_uid unix ~map:unix_map pck uid >>= fun path ->
    let raw = Cd.make_raw ~weight in
    Cd.of_uid unix ~map:unix_map pck raw uid >>= fun v ->
    let[@warning "-8"] off :: _ = Cd.path_to_list path in
    pp_stat Fmt.stdout
      Uid.pp uid off (weight :> int) (Cd.len v) pp_kind (Cd.kind v) pp_path path ;
    Unix.close fd0 ; Unix.close fd1 ; unix.Cd.return () in
