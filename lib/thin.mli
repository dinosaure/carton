...
module Make
  ...
  type ('path, 'fd, 'error) fs =
    ...
    (** A record to manipulate a {i file-system}.
       ...
       create a new one. It returns a {i representation} of it which can be manipulated.
        [append] is like {!Unix.write}. It appends the given {i payload} into the opened
       [fd]. It shifts the offset of the opened file by the length of the given {i payload}.
        [map] is like {!Unix.map_file}. It loads a part of the given ['fd] into a
       {!Bigstringaf.t} payload. It always called with valid [pos] and [len] arguments. *)
